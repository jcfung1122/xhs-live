...
def _call_node_bridge(a1, api, data='', method='POST'):
    """通过 Node.js bridge_sign.js 生成 XS 签名（修复 PyExecJS Invalid \\escape 错误）"""
    bridge_path = os.path.join(os.path.dirname(__file__), '..', 'bridge_sign.js')
    input_data = json.dumps({
        'api': api, 'data': data, 'a1': a1, 'method': method
    })
    proc = subprocess.run(
        ['node', bridge_path], input=input_data,
        capture_output=True, text=True, timeout=10, encoding='utf-8'
    )
    if proc.returncode != 0:
        raise RuntimeError('Node bridge stderr: ' + proc.stderr[:200])
    output = proc.stdout
    brace_pos = output.find('{')
    if brace_pos < 0:
        raise RuntimeError('Node bridge returned non-JSON: ' + output[:200])
    return json.loads(output[brace_pos:])


def generate_xs_xs_common(a1, api, data='', method='POST'):
    try:
        ret = _call_node_bridge(a1, api, data, method)
    except Exception:
        traceback.print_exc()
        print('[WARN] Node bridge failed, falling back to execjs')
...
def _call_node_traceid():
    """通过 Node.js bridge_sign.js 生成 xray traceId（修复 xhs_xray.js 的 Invalid \\escape 错误）"""
    bridge_path = os.path.join(os.path.dirname(__file__), '..', 'bridge_sign.js')
    input_data = json.dumps({'cmd': 'traceid'})
    proc = subprocess.run(
        ['node', bridge_path], input=input_data,
        capture_output=True, text=True, timeout=10, encoding='utf-8'
    )
    if proc.returncode != 0:
        raise RuntimeError('Node bridge traceid stderr: ' + proc.stderr[:200])
    output = proc.stdout
    brace_pos = output.find('{')
    if brace_pos < 0:
        raise RuntimeError('Node bridge traceid non-JSON: ' + output[:200])
    data = json.loads(output[brace_pos:])
    tid = data.get('traceId', '')
    if not tid:
        raise RuntimeError('Node bridge traceid empty')
    return tid


def generate_xray_traceid():
    try:
        return _call_node_traceid()
    except Exception:
        traceback.print_exc()
        print('[WARN] Node bridge traceid failed, falling back to execjs')
