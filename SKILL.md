...
穿透商家获取商品信息后，下载商品图与生成合作买手表（看同行素材收集）：

```bash
python download_competing_images.py <seller_id> <店铺名> [数量]
# 参数: --list 仅展示排序列表 / --all 跳过确认全下 / --no-buyers 不生成买手表
```

**流程**:
1. 商品按【销售额 + 合作买手数】降序排序，先展示列表供用户确认
2. 用户选择序号（如 1,3,5 或 1-10）或回车全下
3. 首次下载时自动生成该店【合作买手表】（按本店合作销售额降序，xlsx）
4. 下载商品图到 `竞品/<店铺名>/`

**保存位置**: `竞品/<店铺名>/`
- 图片: `商品名_¥价格_佣金XX%.{jpg|png}`（扩展名自动识别）
- 表格: `<店铺名>_合作买手.xlsx`（排名/昵称/粉丝/城市/场均销售/本店合作销售额/合作商品数/新买手/佣金意愿）
